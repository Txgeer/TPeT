...
addLayer("p", {
    ...
    upgrades:{       
        ...
        23:{
            ...
            description:"中微子加成自身。",
            effect(){
                let effect = player.points.add(1).log(10).add(1)
                if(hasUpgrade("h",23)) effect = player.points.add(1).log(6).add(1)
                if(hasUpgrade("he",22)) effect = player.points.add(1).log(2).add(1)
                if(hasUpgrade("p",31)) effect = effect.mul(upgradeEffect("p",31))
                return effect
            },
            effectDisplay(){return "x"+format(this.effect())},
        ...
        44:{
            ...
            effect(){
                let effect = two
                if(!hasUpgrade("he",13)) effect = effect.pow(player.points.add(1).log(10).root(2))
                if(hasUpgrade("he",13)) effect = effect.pow(player.points.add(1).log(10).add(1))
                return effect
            },
            effectDisplay(){return "x"+format(this.effect())},
            currencyDisplayName:"中微子",
        ...
        52:{
            ...
            description:" 锂加成氢。",
            effect(){
                let effect = player.li.points.root(4).add(1)
                if(hasUpgrade("p",62)) effect = effect.mul(upgradeEffect("p",62))
                if(effect.gte(10)&&!hasUpgrade("p",68)) effect = effect.root(8).add(10)
                return effect
            },
            effectDisplay(){return "x"+format(this.effect())},
            currencyDisplayName:"中微子",
        ...
        68: {
            title: "进一步的突破......",
            description: "移除 加速生产 VIIII 的软上限。",
        ...
        87: {
            ...
            effectDisplay(){return "x"+format(this.effect())},
            unlocked() { return hasUpgrade("p",86)&&hasUpgrade("p",88); },
            style() {
                if (hasUpgrade(this.layer, this.id)) {
                    return {};
                } else if (canAffordUpgrade(this.layer, this.id)) {
        ...
        94: {
            ...
            effect() {
                return player.p.radiation.add(1)
            },
            effectDisplay() { return "x" + format(this.effect()); },
            style() {
                if (hasUpgrade(this.layer, this.id)) {
        ...
        97: {
            title: "光子的中点",
            description: "自动获取转化时的 100% 光子 /s，并让光子和硼加成温度点。",
            cost: new Decimal(2e10),
            currencyDisplayName: "光子",
            currencyInternalName: "photons",
            currencyLayer: "p",
            effect(){
                let effect = player.p.photons.pow(player.b.points).add(1)
                return effect
            },
            effectDisplay(){return "x"+format(this.effect())},
            unlocked() { return hasUpgrade("p",96)&&hasUpgrade("p",98); },
            style() {
                if (hasUpgrade(this.layer, this.id)) {
                    return {};
                } else if (canAffordUpgrade(this.layer, this.id)) {
    ...
    buyables:{
        11: {
            ...
            effect(x){
                if(hasUpgrade("h",51)){
                    ...
                    if(!hasUpgrade("li",111))a = powsoftcap(a,n(1e30),five)
                    if(hasUpgrade("p",67)&&getBuyableAmount("p",14).gte(1)) a = a.mul(buyableEffect("p",14))
                    if(hasUpgrade("o",32)) a = a.mul(buyableEffect("p",13))
                    return a
                }
                else {
        ...
        14: {
            ...
            effect(x){
                let a = x.pow(1.66686).add(1)
                return a
            },
            purchaseLimit(){
                let a = ten
                if(hasAchievement("a",44)) a = Infinity
                return a
            },
    ...
    clickables: {
        ...
        21: {
            ...
            unlocked() { return true; },
            canClick() { return player.p.electrons.gte(1e9); },
            onClick() {
                let gain = player.points.pow(player.p.electrons);
                if (hasUpgrade("p", 92)) gain = gain.pow(upgradeEffect("p", 92));
                if (hasUpgrade("p", 94)) gain = gain.pow(upgradeEffect("p", 94));
...
addLayer("h", {
    ...
    gainMult() {
        ...
        if(hasUpgrade("p",63)&&!hasMilestone("c",6)) mult = mult.mul(upgradeEffect("p",63))
        if(hasUpgrade("p",63)&&hasMilestone("c",6)) mult = mult.div(upgradeEffect("p",63))
    ...
    passiveGeneration() { 
        ...
        if(hasMilestone("li",8)) a = a.add(1)
        return a
    },
    upgrades:{
        11:{
            ...
            effect(){
                let effect = one
                if(!hasUpgrade("o",21)) effect = effect.add(player.h.points.add(1).mul(10).log(10))
                if(hasUpgrade("o",21)) effect = effect.add(player.h.points.add(1).ln().add(1))
                return effect
            },
            effectDisplay(){return "x"+format(this.effect())},
            cost: new Decimal(1),
        ...
        45:{
            ...
            effect(){
                let effect = player.h.power.add(1).log(10).add(1)
                return effect
            },
            effectDisplay(){return "+"+formatTime(this.effect())},
            cost: new Decimal(81),
    ...
    clickables:{
        ...
        21: {
            title: "爆炸气球！",
            display() {
                if (hasMilestone("c",8)) return "爆炸气球已被移除。";
        ...
        22: {
            title: "爆炸氦气球！",
            display() {
                if (hasMilestone("c",9)) return "爆炸氦气球已被移除。";
            ...
            style() { 
                return { 
                    'background-color': this.canClick() ? "#FFBBDD" : "#BF8F8F",
                };
            },
            unlocked() {return hasMilestone("h",4)},
        },
    ...
    update(diff){
        ...
        if(hasMilestone("li",9)&&player.p.buyableAutobuy) {
            if(hasUpgrade("p",54)){
                let amount11 = player.p.points.root(2).add(1).ssqrt().pow(2).floor().add(1)
                let amount12 = player.p.points.add(1).ssqrt().floor().add(1)
    ...
    balloonBoostH(){
        let mult = player.h.balloon.add(1).pow(2).add(1)
        if(hasUpgrade("h",34)&&!hasUpgrade("li",112)) mult = player.h.balloon.add(1).pow(3).add(1)
        if(hasUpgrade("li",112)) mult = player.h.balloon.pow(player.h.balloon.add(1).log2().add(1)).add(1)
        return mult
    },
    HpowerGet(){
    ...
    addUpTime(){
        let t = player.h.balloon.floor().mul(10)
        return t
    },
    boomedBalloonBoostH(){
        let mult = four 
    ...
    tabFormat: {
        ...
        "气球": {
            content: [
                ...
                "clickables",
                ["display-text",function(){
                    let a = "你有 <span style='color:#FF66DD;text-shadow:0 0 10px'>"+format(player.h.power)+"</span> 氢能"
                    if (!hasUpgrade('c', 13) && layers.h.HpowerGet().gt(layers.h.HpowerGetsoftcap1start())) a = a + "（受软上限限制）"
                    return a
                }],
                ["display-text",function(){
                    let a = "你有 <span style='color:#FF66DD;text-shadow:0 0 10px'>"+format(player.h.balloon)+"</span> 气球"
                    let b = ""
                    if(hasMilestone("h",3)) b = "，气球加成氢 <span style='color:#FF66DD;text-shadow:0 0 10px'>"+format(layers.h.balloonBoostH())+"</span> 倍"
                    return a + b
                }],
                ["display-text",function(){
                    let a = ""
                    let b = ""
                    if(hasUpgrade("h",43)) a = "你有 <span style='color:#FFBBDD;text-shadow:0 0 10px'>"+format(player.he.balloon)+"</span> 氦气球"
                    if(hasMilestone("h",4)) b = "，氦气球加成中微子 <span style='color:#FFFFFF;text-shadow:0 0 10px'>"+format(layers.he.balloonBoostPoints())+"</span> 倍"
                    return a + b
                }],
                ["display-text",function(){
                    let a = ""
                    if(hasMilestone("n",6)) a = "你有 <span style='color:#000000;text-shadow:0 0 10px'>"+format(player.n.balloon)+"</span> 氮气球，氮气球加成游戏速度 <span style='color:#777777;text-shadow:0 0 10px'>"+format(tmp.speedMult)+"</span> 倍"
                    return a
                }],
                ["display-text",function(){
                    let a = ""
...
addLayer("he", {
    ...
    upgrades:{
        11:{
            ...
            description:" 氦加成氢。",
            effect(){
                let effect = player.he.points.add(1).pow(2).add(1)
                return effect
            },
            effectDisplay(){return "x"+format(this.effect())},
            cost: new Decimal(1),
        ...
        13:{
            title:"宇宙本源",
            description:" 解锁更多中微子升级，并优化 粒子降温 的公式。",
            cost: new Decimal(671),
            unlocked(){return hasUpgrade("he",63)},
        },
        14:{
            title:"同位科技",
            description:" 解锁更多锂升级。",
            cost: new Decimal(771),
            unlocked(){return hasUpgrade("he",63)},
        },
        21:{
            title:"氦电子 1s1",
        ...
        53:{
            ...
            cost: new Decimal(3.14e16),
            effect(){
                let effect = player.he.temPoint.add(1).log(2).pow(1.3).add(1)
                return effect
            },
            effectDisplay(){return "x"+format(this.effect())},
        ...
        55:{
            ...
            effect(){
                let effect = player.h.points.add(1).log(2).pow(5).add(1)
                return effect
            },
            effectDisplay(){return "x"+format(this.effect())},
        ...
        61:{
            ...
            effect(){
                let effect = player.be.points.add(3).log(3).pow(2).add(1).mul(player.li.points)
                return effect
            },
            effectDisplay(){return "x"+format(this.effect())},
        ...
        62:{
            ...
            effect(){
                let effect = player.he.temPoint.add(1).log(10).mul(2)
                return effect
            },
            effectDisplay(){return "x"+format(this.effect())},
            unlocked(){return hasUpgrade("he",55)&&hasMilestone("be",3)},
    ...
    buyables:{
        11:{
            ...
            display() {
                if (hasMilestone("c", 12)) return "氦-3 已被移除。";
                return "加成氢能。<br>价格：" + format(this.cost()) + "氦（不消耗）<br>当前数量：" + format(getBuyableAmount(this.layer, this.id)) + "<br>当前效果：" + format(this.effect()) + "x<br>上限数量：" + format(this.purchaseLimit())
            },
            canAfford() {
                if (hasMilestone("c", 12)) return false;
                return player.he.points.gte(this.cost())
            },
            buy() {
                setBuyableAmount(this.layer, this.id, getBuyableAmount(this.layer, this.id).add(1))
            },
            effect(x){
                if (hasMilestone("c", 12)) return one;
                let a = x.add(1)
                if(hasUpgrade("he",33)) a = a.pow(1.5)
                return a
            },
            purchaseLimit(){
                if (hasUpgrade("p", 98)) return new Decimal(Infinity);
                return n(100);
            },
        ...
        12:{
            ...
            display() {
                if (hasMilestone("c", 11)) return "氦-4 已被移除。";
                return "加成氢。<br>价格：" + format(this.cost()) + "氦（不消耗）<br>当前数量：" + format(getBuyableAmount(this.layer, this.id)) + "<br>当前效果：" + format(this.effect()) + "x<br>上限数量：" + format(this.purchaseLimit())
            },
            canAfford() {
                if (hasMilestone("c", 11)) return false;
                return player.he.points.gte(this.cost())
            },
            buy() {
                setBuyableAmount(this.layer, this.id, getBuyableAmount(this.layer, this.id).add(1))
            ...
            effect(x){
                if (hasMilestone("c", 11)) return one;
                let a = x.pow(2).add(1)
                if(hasUpgrade("he",34)) a = x.pow(2).add(x.mul(50).add(1))
                        return a
            },
            purchaseLimit(){
                if (hasUpgrade("p", 74)) return new Decimal(Infinity);
                return n(100);
            },
            unlocked(){return hasMilestone("h",2)},
        },
    ...
    milestones:{
        ...
        14:{
            requirementDescription: "回调后氦温度低于 8e19",
            effectDescription: "解锁 温度点效果|帝江。",
            done(){return hasMilestone("n",3)&&player.he.temperature.lte(8e19)},
            unlocked(){return hasMilestone("he",13)},
        },
        15:{
            requirementDescription: "回调后氦温度低于 2.2e19",
            effectDescription: "超级加成 温度点效果|祝融。",
    ...
    clickables:{
        11:{
            ...
            style() { return { 'background-color': this.canClick()?"#FFDD00":"#BF8F8F" } },
        },
        12:{
            title:"提升冷却氦",
    ...
    addUpTime(){//气球炸的时间
        let t = player.he.balloon.floor().mul(10)
        return t
    },
    ...
    temPointBoostHpower(){//温度点加氢能
        let mult = player.he.temPoint
        if(!hasUpgrade("li",112)) mult = mult.add(1).log(10).add(1).pow(3).add(1)
        if(hasUpgrade("li",112)) mult = pala(player.he.temPoint, player.he.temPoint.add(1).log2().add(1).log2().add(1).floor()).add(1).log2().add(1)
    ...
    temPointdivLicost(){//温度点减锂价格
        ...
        if(!hasUpgrade("li",102)) divt = divt.add(1).log(1.7).mul(3)
    ...
    temPointEffect6(){//温度点加铍
        ...
        if (!hasUpgrade("o",11)) num = num.div(1e140)
        num = num.add(1).log2().add(1)
        return num
    },
    temPointEffect7(){//温度点加电能上限
        let num = player.he.temPoint
        if (!hasUpgrade("o",12)) num = num.div(1e240)
        num = num.add(1).log10().add(1)
        return num
    },
    temPointEffect8(){//温度点加硼烷产能
        let num = player.he.temPoint
        if (!hasUpgrade("o",13)) num = num.div(1e300)
        num = num.add(1).ln().add(1)
        return num
    },
            ...
            content: [
                ...
                ["display-text",function(){
                    ...
                    if(hasMilestone("he",1)) {
                        a = a + "温度点效果|蓐收：加成氢 <span style='color:#FF66DD;text-shadow:0 0 10px'> "+format(layers.he.temPointBoostH())+"</span> 倍"
                    ...
                    if(hasMilestone("he",2)) {
                        a = a + "温度点效果|句芒：降低氦价格 / <span style='color:#FFBBDD;text-shadow:0 0 10px'> "+format(layers.he.temPointdivHecost())+"</span>"
                        if(!hasUpgrade("b",23) && layers.he.temPointdivHecost().gte(layers.he.temPointEffect2SoftcapStart())) a = a + "（受软上限限制）"
                        a = a + "<br>"
                    }
                    if(hasMilestone("he",3)) {
                        a = a + "温度点效果|共工：加成中微子 <span style='color:#FFFFFF;text-shadow:0 0 10px'> "+format(layers.he.temPointBoostPoints())+"</span> 倍"
                        if (!hasUpgrade('h', 52) && layers.he.temPointBoostPoints().gte(layers.he.temPointEffect3SoftcapStart())) a = a + "（受软上限限制）"
                        a = a + "<br>"     
                    }  
                    if(hasMilestone("he",4)) {
                        a = a + "温度点效果|祝融：降低锂价格 / <span style='color:#DD0033;text-shadow:0 0 10px'> "+format(layers.he.temPointdivLicost().add(1))+"</span>"
                        a = a + "<br>"     
                    }     
                    if(hasMilestone("he",8)) {
                        a = a + "温度点效果|后土：加成氢能 <span style='color:#FF66DD;text-shadow:0 0 10px'> "+format(layers.he.temPointBoostHpower())+"</span> 倍"
                        //if(layers.he.temPointBoostHpower().gte(layers.he.temPointEffect5SoftcapStart())) a = a + "（已达软上限）"
                        a = a + "<br>"     
                    }  
                    if(hasMilestone("he",10)) {
                        a = a + "温度点效果|天吴：加成铍 <span style='color:#55CC77;text-shadow:0 0 10px'> "+format(layers.he.temPointEffect6())+"</span> 倍"
                        //if(layers.he.temPointEffect6().gte(layers.he.temPointEffect6SoftcapStart())) a = a + "（受软上限限制）"
                        a = a + "<br>"     
                    }   
                    if(hasMilestone("he",11)) {
                        a = a + "温度点效果|玄冥：加成电能上限 <span style='color:#DDDD33;text-shadow:0 0 10px'> "+format(layers.he.temPointEffect7())+"</span> 倍"
                        a = a + "<br>"     
                    }
                    if(hasMilestone("he",12)) {
                        a = a + "温度点效果|强良：加成硼烷产能 <span style='color:#992222;text-shadow:0 0 10px'> "+format(layers.he.temPointEffect8())+"</span> 倍"
                        a = a + "<br>"     
                    }
...
addLayer("li", {
    ...
    gainMult() {
        ...
        if(hasUpgrade("li",72)) mult = mult.div(upgradeEffect("li",72))
        if(hasUpgrade("li",114)) mult = mult.div(upgradeEffect("li",114))
        if(hasMilestone("be",1)) mult = mult.div(2)
    ...
    upgrades:{
        ...
        51:{
            ...
            cost: new Decimal(4),
            effect(){
                let effect = player.li.currentElectricity
                if(!hasUpgrade("o",23)) effect = effect.add(1).pow(6).add(1)
                if(hasUpgrade("o",23)) effect = effect.add(1).pow(player.li.currentElectricity.add(1).log2().add(1).log2().add(1)).add(1)
                return effect
            },
            effectDisplay(){return "/"+format(this.effect())},
            unlocked(){return hasUpgrade("li",31)},
        ...
        71:{
            title:"研究-51",
            description:"解锁新的电子升级，且 绿石科技 效果更改为基于总研究点。",
        ...
        81:{
            ...
            unlocked(){return hasUpgrade("li",71)&&hasUpgrade("li",72) },
            effect(){
            ...
            currencyLayer:"li",
            canAfford(){
                let canbuy = hasUpgrade("li",71)&&hasUpgrade("li",72);
                return canbuy
            },        
        },
        82: {
            ...
            currencyLayer: "li",
            canAfford() {
                return hasUpgrade("li",71)&&hasUpgrade("li",72);
            },
        },
        91: {
            title: "研究-71",
        ...
        111: {
            ...
            currencyLayer: "li",
            canAfford() {
                return hasUpgrade("li", 101);
            },
        },
        112: {
            title: "研究-b2",
            description: "加成 温度点效果|后土，并进一步优化氢气球和氦气球的公式。",
            cost: new Decimal(40000000),
            unlocked() { return hasUpgrade("li", 102); },
            currencyDisplayName: "研究点",
            currencyInternalName: "researchPoint",
            currencyLayer: "li",
            canAfford() {
        ...
        114: {
            ...
            effectDisplay() { return "/" + format(this.effect()); },
            unlocked() { return hasUpgrade("li", 104); },
            currencyDisplayName: "研究点",
            currencyInternalName: "researchPoint",
            currencyLayer: "li",
            canAfford() {
    ...
    buyables:{
        11:{
            ...
            unlocked(){return hasMilestone("be",1)},
            style() { return { 'background-color': this.canAfford()?"#DD0033":"#BF8F8F" }},
        },
        12:{
            title: "电研究点",
            cost(x) {
        ...
        31: {
            ...
            display() { 
                if (hasMilestone("c", 3)) return "电容增幅已被移除。";
                return "重置所有电能，加成电能上限。<br>价格：" + format(this.cost()) + "电能<br>当前数量：" + format(getBuyableAmount(this.layer, this.id)) + "<br>当前效果：" + format(this.effect()) + "x<br>上限数量：" + format(this.purchaseLimit())
            },
            effect(x){
                if (hasMilestone("c", 3)) return one;
                let eff = x.add(1).pow(2)
                return eff
            },
            canAfford() { 
                if (hasMilestone("c", 3)) return false;
    ...
    totalResearchPoints() {
        ...
        let base;
        if (hasUpgrade("li", 62)) {
            let types = [li, dian, shi];
            if (hasUpgrade("p", 81)) {
                types.push(guang);
            }
            base = one;
            for (let i = 0; i < types.length; i++) {
                base = base.mul(types[i]);
            }
        } else {
    ...
    tabFormat:{
        "主页": {   
            content: [
                ...
                "prestige-button",   
                ["display-text",
                    function(){return "你有 <span style='color:#FF66DD;text-shadow:0 0 10px'>"+format(player.h.points)+"</span> 氢"}],
                ["display-text",
                    function(){
                        let a = "你的锂加成氢获取 <span style='color:#FF66DD;text-shadow:0 0 10px'>"+format(layers.li.LiboostH())+"</span>x 加成氢能获取 <span style='color:#FF66DD;text-shadow:0 0 10px'>"+format(layers.li.LiboostHpower())+"</span>x 降低氦价格 / <span style='color:#FFBBDD;text-shadow:0 0 10px'>"+format(layers.li.LidivHecost())+"</span>"
                        if (hasMilestone("li",6)) a = a + "<br>加成温度点获取 <span style='color:#FFDD00;text-shadow:0 0 10px'>"+format(layers.li.LiboostTemPoint())+"</span>x"
        ...
        "电池": {
            content: [
                ...
                ["display-text", function() {return "你有 <span style='color:#FF66DD;text-shadow:0 0 10px'>"+format(player.h.power)+"</span> 氢能，每秒生产 <span style='color:#DDDD33;text-shadow:0 0 10px'>"+format(layers.li.electricityGain())+"</span> 电能（至少需要 1e60 氢能）"}],
                ["display-text", function() {
...
addLayer("be", {
    ...
    gainMult() {
        ...
        if(hasMilestone("he",10)) mult = mult.mul(layers.he.temPointEffect6())
    ...
    clickables: {
        21: {
            ...
            onClick: function() {
                ...
                player.p.electrons = new Decimal(0);
                player.h.points = new Decimal(0);
                player.h.power = new Decimal(0);
                player.h.balloon = new Decimal(0);
                player.he.points = new Decimal(0);
                player.he.balloon = new Decimal(0);
                player.he.temPoint = new Decimal(0);
                player.li.points = new Decimal(0);
                player.li.currentElectricity = new Decimal(0);
                player.be.points = new Decimal(0);
    ...
    tabFormat:{
        "主页": {   
            content: [
                ...
                ["display-text",
                    function(){return "你有 <span style='color:#FFBBDD;text-shadow:0 0 10px'>"+format(player.he.points)+"</span> 氦"}],
                "milestones"
            ],
            unlocked(){return player.be.unlocked}
        },
...
addLayer("b", {
    ...
    doReset: function() {
        ...
        if (!player[this.layer].unlocked) {
            ...
            needCanvasUpdate = true;
            player[this.layer].justUnlocked = true;
            setTimeout(() => {
                if (player[this.layer]) player[this.layer].justUnlocked = false;
            }, 2000);
            // 若有 unlockOrder 需要处理，可按需添加
        }
        // ===== 原有代码继续 =====
    ...
    upgrades:{
        ...
        25:{
            ...
            effect(){
                let effect = player.b.points.add(1)
                return effect
            },
            effectDisplay(){return "x"+format(this.effect())},
            cost: new Decimal(5000),
        ...
        71:{
            title:"氨硼烷助力光子",
            description:" 氨硼烷加成光子获取。",
            effect(){
                let effect = player.b.borane6.add(1).log2().add(1).floor()
                return effect
            },
            effectDisplay(){return "x"+format(this.effect())},
            cost: new Decimal(3000000),
        ...
        72:{
            ...
            description:" 氨硼烷加成碳。",
            effect(){
                let effect = player.b.borane6.add(1).pow(0.6).add(1)
                return effect
            },
            effectDisplay(){return "x"+format(this.effect())},
        ...
        73:{
            ...
            description:" 氨硼烷加成铍。",
            effect(){
                let effect = player.b.borane6.add(1).pow(0.7).add(1)
                return effect
            },
            effectDisplay(){return "x"+format(this.effect())},
        ...
        74:{
            ...
            description:" 氨硼烷加成氢。",
            effect(){
                let effect = player.b.borane6.add(1).pow(0.8).add(1)
                return effect
            },
            effectDisplay(){return "x"+format(this.effect())},
        ...
        75:{
            title:"氨硼烷终极升级",
            description:" 氨硼烷加成中微子。",
            effect(){
                let effect = player.b.borane6.add(1).pow(0.9).add(1)
                return effect
            },
            effectDisplay(){return "x"+format(this.effect())},
    ...
    chooseBoraneGainMax(){
        let num = 1;
        if (hasUpgrade("b",24)&&!hasMilestone("c",5)) num += 1;
        if (hasUpgrade("n",23)) num = 6;
        return num;
    },
    update(diff){
        // ---- 全面防御性初始化 ----
    ...
    tabFormat:{
        "里程碑": {   
            content: [
                ...
                "prestige-button",
                ["display-text",
                    function(){return "你有 <span style='color:#FFDD00;text-shadow:0 0 10px'>"+format(player.he.temPoint)+"</span> 温度点"}],
                "milestones"
            ],
            unlocked(){return player.b.unlocked}
        },
        ...
        "升级": {   
            content: [
                ...
                ["display-text",function(){
                    ...
                    function prodGlow(color, num) {
                        const producing = (player.b.gainBorane || []).includes(num);
                        if (producing) {
                            return "text-shadow:0 0 20px " + color + ";";
                        }
                    }

                    if(hasMilestone("b",3))text1 = "你有 <span style='color:#FF3F3F;"+prodGlow("#FF3F3F",1)+"'>"+format(player.b.borane1)+"</span> 癸硼烷<br>你的癸硼烷产量为 <span style='color:#FF3F3F;"+prodGlow("#FF3F3F",1)+"'>"+format(layers.b.boraneGain(1))+"</span> / s"
                    if(hasMilestone("b",4))text2 = "你有 <span style='color:#FF7F3F;"+prodGlow("#FF7F3F",2)+"'>"+format(player.b.borane2)+"</span> 己硼烷<br>你的己硼烷产量为 <span style='color:#FF7F3F;"+prodGlow("#FF7F3F",2)+"'>"+format(layers.b.boraneGain(2))+"</span> / s"
...
addLayer("c", {
    ...
    update(diff){
        let count = getBuyableAmount('c', 11).toNumber();
        if (hasMilestone("c", 10) && count > 0) count = 1;
        if (count > 0) player.c.entropy = player.c.entropy.add(layers.c.getEntropyGain().mul(count).mul(diff));
    },
    upgrades:{
        11:{
            title:"CCB",
    ...
    buyables: {
        11: {
            ...
            effect(x) {
                if (hasMilestone("c", 10) && x.gt(0)) return one;
                return x;
            },
            purchaseLimit() {
                if (hasUpgrade("p", 84)) return new Decimal(Infinity);
    ...
    milestones:{
        1:{
            requirementDescription: "765 原油",
            effectDescription: "减益：氢能 / √8；解锁石油气与新的电子升级。",
            done(){return player.c.oil.gte(765)},
            unlocked(){return player.c.oil.gte(1)},
        },
        2:{
            requirementDescription: "865 原油",
            effectDescription: "减益：氦的指数+0.25；解锁石油醚。",
            done(){return player.c.oil.gte(865)},
            unlocked(){return hasMilestone("c",1)},
        },
        3:{
            requirementDescription: "1215 原油",
            effectDescription: "减益：移除 电容增幅；解锁汽油。",
            done(){return player.c.oil.gte(1215)},
            unlocked(){return hasMilestone("c",2)},
        },
        4:{
            requirementDescription: "1295 原油",
            effectDescription: "减益：反转 电子加速 的效果；解锁煤油。",
            done(){return player.c.oil.gte(1295)},
        ...
        10:{
            requirementDescription: "7777 原油",
            effectDescription: "减益：自动点击者的点击倍率在任何时候都为1；解锁人造原油。",
            done(){return player.c.oil.gte(7777)},
            unlocked(){return hasMilestone("c",9)},
        },
        11:{
            requirementDescription: "10234 原油",
            effectDescription: "减益：移除 氦-4；解锁煤焦油。",
            done(){return player.c.oil.gte(10234)},
            unlocked(){return hasMilestone("c",10)},
        },
        12:{
            requirementDescription: "11451 原油",
            effectDescription: "减益：移除 氦-3；解锁木焦油和新的氧升级。",
            done(){return player.c.oil.gte(11451)},
            unlocked(){return hasMilestone("c",11)},
        },
        13:{
            requirementDescription: "17900 原油",
            effectDescription: "减益：碳的指数-0.25；解锁粗氨水和新的氧升级。",
            done(){return player.c.oil.gte(17900)},
            unlocked(){return hasMilestone("c",12)},
        },
        14:{
            requirementDescription: "1500000 原油",
            effectDescription: "减益：削弱 温度点效果|蓐收；解锁精氨水和合成氨。",
    ...
    tabFormat: {
        "主页": {
            content: [
                ...
                "prestige-button",
                ["display-text",
                    function(){ return "你有 <span style='color:#FF66DD;text-shadow:0 0 10px'>"+format(player.h.power)+"</span> 氢能"; }
                ],
                ["upgrades",[1]]
            ],
            unlocked(){ return true; }
        ...
        "原油": {
            content: [
                ...
                ["display-text",
                    function(){ if(hasMilestone("c",6)) return "润滑油：降低锂价格 / <span style='color:#DD0033;text-shadow:0 0 10px'>"+format(player.c.entropy.pow(player.c.entropy.add(1).ln().add(1)))+"</span> "; }
                ],
                ["display-text",
...
addLayer("n", {
    ...
    milestones:{
        ...
        7:{
            requirementDescription: "10000000 氮",
            effectDescription: "解锁新的光子升级。",
            done(){return player.n.points.gte(10000000)},
            unlocked(){return hasMilestone("n",6)},
        },
        8:{
    ...
    upgrades:{
        ...
        22: {
            title:"硝酸",
            description:"电子和光子加成碳。",
            cost: new Decimal("1e3008"),
            effect(){
                let effect = player.p.electrons.pow(player.p.photons).add(1).log2().add(1)
                return effect
            },
            effectDisplay(){return "x"+format(this.effect())},
        ...
        24: {
            title:"次硝酸",
            description:"氮加成熵。",
            cost: new Decimal("1e3041"),
            effect(){
                let effect = player.n.points.add(1).log2().add(1)
                return effect
            },
            effectDisplay(){return "x"+format(this.effect())},
    ...
    clickables: {
        11: {
            ...
            display() {
                ...
                let allDone = player.n.hydrogenInjected
                    ...
                    && player.n.temperatureProvided
                    && player.n.pressureProvided;
                if (allDone) {
                    let progress = Math.floor((player.n.ammoniaHoldTime || 0) * 100);
                    return "长按 1 秒以合成第 " + targetTier + " 层氨！<br>当前进度：<span style='color:#FFFFFF;text-shadow:0 0 10px'>" + progress + "%</span>";
                }
                return "请先提供四种原料，以准备第 " + targetTier + " 层合成氨。";
            },
            unlocked() { return true; },
            canClick() {
            ...
            onHold() {
                ...
                if (player.n.ammoniaHoldTime >= 1) {
                    ...
                    player.n.temperatureProvided = false;
        ...
        21: {
            ...
            onClick() {
                ...
                player.h.points = player.h.points.sub(req.h).max(0);
            },
            style() { return { 'background-color': this.canClick()?"#FF66DD":"#BF8F8F"} }
        },
        22: {
            title: "注入氮气",
            display() {
            ...
            onClick() {
                ...
                player.n.points = player.n.points.sub(req.n).max(0);
            },
        },
        23: {
            title: "提供温度",
            ...
            onClick() {
                ...
                player.c.points = player.c.points.sub(req.c).max(0);
            },
            style() { return { 'background-color': this.canClick()?"#555555":"#BF8F8F"} }
        },
        24: {
            title: "提供气压",
            display() {
            ...
            onClick() {
                ...
                player.n.pressureProvided = true;
                player.o.points = player.o.points.sub(req.o).max(0);
            },
            style() { return { 'background-color': this.canClick()?"#BBDDFF":"#BF8F8F"} }
        },
    },
    ammoniaRequirement(tier) {
        const table = {
            1: { h: "1e21700", n: "1e3000", c: "1e2800", o: "30000" },
    ...
    update(diff){
        if (hasUpgrade("n", 21)) {
            ...
            if (gain.lt(1)) gain = zero;
            if (gain.gt(0)) {
                player.b.transcendCrystals = player.b.transcendCrystals.add(gain.mul(diff));
            }
        }
        if (hasUpgrade("n", 23) && player.b) {
            if (!Array.isArray(player.b.gainBorane) || player.b.gainBorane.length < 6) {
                player.b.gainBorane = [1, 2, 3, 4, 5, 6];
            }
        }
    },
    tabFormat: {
...
addLayer("o", {
    ...
    upgrades:{
        11:{
            title:"氧-16",
            description:"加成 温度点效果|天吴。",
            cost: new Decimal(100),
            unlocked(){return hasMilestone("n",9)},
        },
        12:{
            title:"氧-17",
            description:"加成 温度点效果|玄冥。",
            cost: new Decimal(200),
            unlocked(){return hasUpgrade("o",11)},
        },
        13:{
            title:"氧-18",
            description:"加成 温度点效果|强良。",
            cost: new Decimal(300),
        ...
        31:{
            title:"氧化-硼",
            description:"放射性降低硼价格。",
            cost: new Decimal(5000),
            effect(){
                let effect = pala(player.p.radiation,player.p.radiation.add(1).log10().add(1).log10().add(1).log10().add(1).floor())
                return effect
            },
            effectDisplay(){return "/"+format(this.effect())},
            unlocked(){return hasMilestone("c",13)},
        ...
        32:{
            title:"氧化-碳",
            description:"氦和锂加成碳，并让 粒子加速器|相织 也能加成 粒子加速器|原初。",
            cost: new Decimal(10000),
            effect(){
                let effect = player.he.points.mul(player.li.points).add(1)
                return effect
            },
            effectDisplay(){return "x"+format(this.effect())},
    ...
    achievements: {
        ...
        14: {
            ...
            effect() {
                ...
                if (hasAchievement('a', 42)) len = len * achievementEffect('a', 42);
        ...
        23: {
            ...
            tooltip: "要求：获得 10 研究点。<br>奖励：成就 14 的效果也同样加成电能上限，并让电能流失速度降低 0.4%。", 
            effect() {
                let len = getAchievementCount();
        ...
        41: {
            name: "天嘿了",
            done() {return player.n.points.gte(3000)},
            tooltip: function() {
                if (hasAchievement(this.layer, this.id)) {
                    let eff = achievementEffect(this.layer, this.id);
                    return `要求：获得 3000 氮。<br>奖励：已完成的成就个数加成硼烷产能。<br>当前：x${format(eff)}`;
                } else {
                    return `要求：获得 3000 氮。<br>奖励：已完成的成就个数加成硼烷产能。<br>当前：x1.00`;
                }
            },
            effect() {
                let len = getAchievementCount();
        ...
        42: {
            ...
            effect() {
                return player.c.oil.add(1);
            },
            unlocked() {return hasAchievement("a",35)}
        },
        43: {
            name: "一个元-成就",
            done() {return getAchievementCount() >= 3240},
            tooltip: function() {
                if (hasAchievement(this.layer, this.id)) {
                    let eff = achievementEffect(this.layer, this.id);
                    return `要求：获得 3240 已完成的成就个数。<br>奖励：已完成的成就个数加成光子。<br>当前：x${format(eff)}`;
                } else {
                    return `要求：获得 3240 已完成的成就个数。<br>奖励：已完成的成就个数加成光子。<br>当前：x1.00`;
                }
            },
            effect() {
                let len = getAchievementCount();
...
addLayer("t", {
    ...
    infoboxes: {
        ...
        "h": {
            ...
            style: { "border-color": "#FF66DD" },
            titleStyle: { "background-color": "#FF66DD" },
            bodyStyle: { "color": "#FFFFFF" }
        },
        "he": {
            title: "氦",
            ...
            style: { "border-color": "#FFBBDD" },
            titleStyle: { "background-color": "#FFBBDD" },
            bodyStyle: { "color": "#FFFFFF" }
        },
        "li": {
            title: "锂",
            ...
            style: { "border-color": "#DD0033" },
            titleStyle: { "background-color": "#DD0033" },
            bodyStyle: { "color": "#FFFFFF" }
        },
        "be": {
            title: "铍",
